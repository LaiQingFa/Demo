...
<div class="inner-block">
    <div class="blank">
    	...
    	<form id="cahxun" action="BookManageAction!BookChaxun.action" method="post" >
			<input type="text" name="bookIDnum" value="${bookIDnum}"  placeholder="Search..." required="">	
      ...
      <table border="0" width="820" class="yujing_cc yjjl4_cc">
              ...
              <tr>
               <td>${tbooklist.bookkkId}</td>
                <td>${tbooklist.bookkkName}</td>
                 <c:if test="${tbooklist.bookkkType==0}"><td>教育类</td></c:if>
                <c:if test="${tbooklist.bookkkType==1}"><td>工商类</td></c:if>
	            <c:if test="${tbooklist.bookkkType==2}"><td>经济类</td></c:if>
		         <td>${tbooklist.bookkkPublisherName}</td>
                <td>${tbooklist.bookkkAuthorName}</td>
                <td>${tbooklist.bookkkPric}</td>
                <c:if test="${tbooklist.bookkkState==0}"><td>存库</td></c:if>
		        <c:if test="${tbooklist.bookkkState==1}"><td>借出</td></c:if>
		        <td>${tbooklist.bookkkStorageTime}</td>
                <td>${tbooklist.bookkkStorageLocation}</td>
                <td>${tbooklist.bookkkMakerrr}</td>
                </tr>
